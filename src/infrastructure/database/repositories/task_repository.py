...
class TaskRepository:
    ...
    # ==========================================================
    # REPORT FILTER SOURCE FROM vwTask
    # ==========================================================
    def get_report_filter_source(
        self,
        owner_ids: Optional[Union[int, List[int], str]] = None,
        ...
        """
        Retorna apenas as colunas necessárias para filtros de Tasks > Reports,
        usando a vwTask como fonte.
        """
        ...
        parsed_owner_ids = self._parse_int_ids(owner_ids)
        if owner_ids is not None:
            if isinstance(owner_ids, (list, tuple, set)) and len(owner_ids) == 0:
                pass
            elif isinstance(owner_ids, str) and owner_ids.strip() == "":
                pass
            elif parsed_owner_ids:
                placeholders = ", ".join(["%s"] * len(parsed_owner_ids))
                conditions.append(f"task_owner_id IN ({placeholders})")
                params.extend(parsed_owner_ids)
            else:
                conditions.append("1=0")

        if task_type_names:
            normalized = [str(v).strip() for v in task_type_names if str(v).strip()]
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                conditions.append(f"task_type_name IN ({placeholders})")
                params.extend(normalized)

        if client_names:
            normalized = [str(v).strip() for v in client_names if str(v).strip()]
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                conditions.append(f"task_customer_name IN ({placeholders})")
                params.extend(normalized)

        if status_names:
            normalized = [str(v).strip() for v in status_names if str(v).strip()]
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                conditions.append(f"task_status_name IN ({placeholders})")
                params.extend(normalized)
        ...
        try:
            ...
            self.error_repo.log_error(
                error_function="TaskRepository.get_report_filter_source",
                error_command=query,
                error_description=str(e),
                error_traceback=traceback.format_exc()
            )
    ...
    # ==========================================================
    # REPORT OWNER OPTIONS FROM vwTask
    # ==========================================================
    def get_report_owner_options(
        self,
        task_type_names: Optional[List[str]] = None,
        client_names: Optional[List[str]] = None,
        status_names: Optional[List[str]] = None,
        ...
        """
        Retorna owners distintos para Tasks > Reports, derivados da vwTask.
        """
        ...
        query = """
            SELECT DISTINCT
                ...
                task_owner_name
            FROM vwTask
            WHERE task_owner_id IS NOT NULL
              AND task_owner_name IS NOT NULL
              AND TRIM(task_owner_name) <> ''
        """
        ...
        if task_type_names:
            ...
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                query += f" AND task_type_name IN ({placeholders})"
                params.extend(normalized)
        ...
        if client_names:
            ...
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                query += f" AND task_customer_name IN ({placeholders})"
                params.extend(normalized)
        ...
        if status_names:
            ...
            if normalized:
                placeholders = ", ".join(["%s"] * len(normalized))
                query += f" AND task_status_name IN ({placeholders})"
                params.extend(normalized)

        query += " ORDER BY task_owner_name"
        ...
        try:
            ...
            self.error_repo.log_error(
                error_function="TaskRepository.get_report_owner_options",
                error_command=query,
                error_description=str(e),
                error_traceback=traceback.format_exc()
            )
