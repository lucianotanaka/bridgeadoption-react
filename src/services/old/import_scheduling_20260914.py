...
from app.services.importers.import_company_name import run_import as run_import_company
from app.services.importers.cisco_subscription_ccw import run_import as run_import_ccw
from app.services.importers.cisco_ready import run_import as run_import_cisco_ready
from app.services.importers.pxp_import_task_6702 import run_import as run_import_cisco_lci_task
from app.services.importers.pxp_import_activity_5890 import run_import as run_import_cisco_lci_activity
from app.services.importers.cisco_smart_account_usage_fetcher import run_import as run_cisco_smart_account_usage_fetcher
from app.services.importers.cisco_enterprise_agreement_usage_fetcher import run_import as run_cisco_enterprise_agreement_usage_fetcher

# ==========================================================
# CONFIGURAÇÕES
# ==========================================================

IMPORT_INPUT_DIR = "/home/bridgeadoption/storage/input"
...
# ==========================================================
# LIMPEZA DE ARQUIVOS EXCEL APÓS IMPORTAÇÃO
# ==========================================================

def cleanup_empty_import_files(jobs: List[dict]) -> None:
    """
    Após o processamento dos jobs, verifica os arquivos Excel utilizados.
    Se o arquivo contiver apenas a linha de cabeçalho (sem dados),
    ele é excluído da pasta de entrada (IMPORT_INPUT_DIR).

    Suporta formatos: .xlsx, .xlsm, .xltx, .xltm
    """
    try:
        import openpyxl
    except ImportError:
        logging.warning(
            "openpyxl não disponível. Limpeza de arquivos Excel não executada."
        )
        return

    # Coleta os nomes de arquivo únicos dos jobs processados
    processed_files: set = set()
    for job in jobs:
        file_name = job.get("importctrl_file")
        if file_name:
    ...
    logging.info(
        "Verificando %s arquivo(s) para limpeza pós-importação.", len(processed_files)
    )

    excel_extensions = {".xlsx", ".xlsm", ".xltx", ".xltm"}

    for file_name in processed_files:
        file_path = os.path.join(IMPORT_INPUT_DIR, file_name)

        if not os.path.isfile(file_path):
            logging.debug("Arquivo não encontrado para limpeza: %s", file_path)
            continue

        ext = os.path.splitext(file_name)[1].lower()
        if ext not in excel_extensions:
            logging.debug(
                "Arquivo ignorado na limpeza (formato não suportado): %s", file_name
            )
            continue
        ...
        try:
            ...
            if max_row is not None and max_row <= 1:
                os.remove(file_path)
                logging.info(
                    "Arquivo Excel excluído (contém apenas cabeçalho): %s", file_path
                )
            else:
                logging.debug(
                    "Arquivo mantido (%s linha(s) encontradas): %s",
                    max_row,
                    file_path,
                )
            ...
            logging.error(
                "Erro ao verificar/excluir arquivo '%s': %s", file_path, e
            )
...
def run() -> int:
    ...
    cleanup_empty_import_files(jobs)
