...
def _build_ea_payload_from_row(
    ...
    return {
        ...
        # Colunas opcionais — incluídas apenas quando presentes no arquivo de origem.
        # Se a coluna não existir no Excel, row.get() retorna None e o campo é omitido do payload.
        **({
            "ea_pending_tf_effective_date": to_dt(row.get("Pending True Forward Effective Date")),
        } if "Pending True Forward Effective Date" in row else {}),
        **({
            "ea_consumed_suite_value_percent": (
                round(to_float(row.get("Consumed Suite Value (%)")) / 100, 8)
                if to_float(row.get("Consumed Suite Value (%)")) is not None
                else None
            ),
        } if "Consumed Suite Value (%)" in row else {}),
        **({
            "ea_exceptional_growth_anniversary": to_dt(row.get("Exceptional Growth Anniversary")),
        } if "Exceptional Growth Anniversary" in row else {}),
        **({
            "ea_exceptional_growth_tf_eligible": norm(row.get("Exceptional Growth TF Eligible")),
        } if "Exceptional Growth TF Eligible" in row else {}),
    }
...
def run_import(file_path: str, user_id: Optional[str] = None, import_control_id: Optional[int] = None):
    """
    Fluxo refatorado:
    ...
    try:
        while ws.max_row > 1:
            ...
            _append_execution_log(
                execution_log_path,
                f"INFO checkpoint_after_chunk chunk={chunk_number} imported_total={imported_rows} error_total={error_rows} remaining_rows_current={remaining_rows_current}",
            )
