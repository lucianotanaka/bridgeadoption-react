...
IMPORT_CONTROL_PROGRESS_INTERVAL_FILES = 1
...
def _report_progress(
    ...
    try:
        ...
        _append_execution_log(log_path, f"WARN failed updating import control progress error={str(e)[:500]}")
...
def _process_file(source_file: Path, import_control_id: Optional[int] = None) -> None:
    _ensure_directories()
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # 7) Logs finais + progresso na tbImportControl
        # ------------------------------------------------------------------
...
def run_import(file_name: Optional[str] = None, user_id: Optional[str] = None, import_control_id: Optional[int] = None) -> Dict[str, str]:
    """
    ...
    for source_file in source_files:
        ...
        _process_file(source_file, import_control_id=import_control_id)
