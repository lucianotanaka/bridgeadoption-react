...
def _build_task_payload(
    ...
    return {
        ...
        "task_priority": "LOW",
        "task_project_id": 0,
        "task_status": TASK_STATUS_IN_PROGRESS if data.get("task_opt_in_flag") == 1 else TASK_STATUS_OPEN,
        ...
        "task_eligible": data.get("task_eligible"),
        "task_end_fy": None,
...
def _update_activity_from_stage(
    ...
    if new_completed is not None and _normalize_compare_decimal(activity.get("activity_completed")) != _normalize_compare_decimal(new_completed):
...
def _sync_task_activities(
    ...
    file_path: str,
    row_number: int,
) -> Tuple[bool, int, Optional[str], Optional[str], Optional[Any]]:
    ...
    return True, updated_count, None, None, None
...
def _process_single_row(
    ...
    try:
        ...
        activities_ok, activities_updated, activities_error, activities_error_column, activities_error_value = _sync_task_activities(
            task_id=task_id,
            data=data,
            file_path=file_path,
            row_number=row_number,
        ...
        if execution_log_path:
            _append_execution_log(
                execution_log_path,
                f"INFO row={row_number} task existing processed task_id={task_id} ws={data.get('ws')} updated={updated} reopened={task_reopened} activities_updated={activities_updated}",
            )

        return RowProcessResult(
            success=True,
            updated=(updated or task_reopened or activities_updated > 0),
            ignored=not (updated or task_reopened or activities_updated > 0),
