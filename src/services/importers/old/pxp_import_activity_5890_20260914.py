...
def _is_duplicate_source_row_still_pending(
    file_path: str,
    current_row_number: int,
    current_activity_ws: Optional[str],
    current_track: Optional[str],
    current_subtrack: Optional[str],
    current_deal_id: Optional[str],
    current_activity_type: Optional[str],
) -> bool:
    if current_row_number <= 2:
        return False

    try:
        wb_scan, ws_scan, idx_map_scan, _ = _open_workbook_rw(file_path)
    except Exception:
        return False

    try:
        for scan_row in range(2, current_row_number):
            row_cells = ws_scan[scan_row]
            raw_values = [cell.value for cell in row_cells]

            scan_activity_ws = _normalize_str(
                raw_values[idx_map_scan["Activity Id"]] if idx_map_scan.get("Activity Id") is not None and idx_map_scan["Activity Id"] < len(raw_values) else None,
                max_length=255,
            )
            scan_track = _normalize_str(
                raw_values[idx_map_scan["Track"]] if idx_map_scan.get("Track") is not None and idx_map_scan["Track"] < len(raw_values) else None,
                max_length=255,
            )
            scan_subtrack = _normalize_str(
                raw_values[idx_map_scan["Sub-Track"]] if idx_map_scan.get("Sub-Track") is not None and idx_map_scan["Sub-Track"] < len(raw_values) else None,
                max_length=255,
            )
            scan_deal_id = _normalize_str(
                raw_values[idx_map_scan["Deal ID"]] if idx_map_scan.get("Deal ID") is not None and idx_map_scan["Deal ID"] < len(raw_values) else None,
                max_length=255,
            )
            scan_activity_type = _normalize_str(
                raw_values[idx_map_scan["Activity Type"]] if idx_map_scan.get("Activity Type") is not None and idx_map_scan["Activity Type"] < len(raw_values) else None,
                max_length=255,
            )

            if (
                scan_activity_ws == current_activity_ws
                and scan_track == current_track
                and scan_subtrack == current_subtrack
                and scan_deal_id == current_deal_id
                and scan_activity_type == current_activity_type
            ):
                return True
    finally:
        wb_scan.close()
...
def _process_single_row(
    ...
    try:
        ...
        if existing_activity is None:
            task_ws = _normalize_str(task_data.get("task_ws")) if task_data else None

            existing_activity = _resolve_existing_activity(
                activity_ws=activity_ws,
                where_fallback=where_fallback,
                parent_task_ws=task_ws,
            )
                    ...
                    if found_activity_id:
                        try:
                            ...
                            if execution_log_path:
                                _append_execution_log(
                                    execution_log_path,
                                    f"INFO row={row_number} activity_ws corrected via fallback: activity_id={found_activity_id} old_ws={current_activity_ws!r} new_ws={activity_ws} task_ws={task_ws}",
                                )
                        except Exception:
                            pass
        ...
        if existing_activity is None:
            if _is_duplicate_source_row_still_pending(
                file_path=file_path,
                current_row_number=row_number,
                current_activity_ws=activity_ws,
                current_track=track,
                current_subtrack=subtrack,
                current_deal_id=deal_id,
                current_activity_type=resolved_activity_type,
            ):
                if execution_log_path:
                    _append_execution_log(
                        execution_log_path,
                        f"INFO row={row_number} duplicate source row deferred activity_ws={activity_ws} task_id={task_id} activity_type={resolved_activity_type}",
                    )
