...
def _build_execution_log_path(source_file: Path) -> Path:
    ...
    return BASE_LOGS_PATH / f"{source_file.stem}__{ts}.log"
...
def _build_failed_file_path(source_file: Path) -> Path:
    return BASE_OUTPUT_PATH / f"{source_file.stem}__failed_rows.xlsx"


def _ensure_failed_workbook(failed_file: Path, failed_headers: List[str]) -> None:
    """
    Garante que o arquivo fixo de falhas exista.
...
def _resolve_customer_id(client_name: Any, customer_cache: Dict[str, Optional[int]]) -> Optional[int]:
    ...
    Esta função é tolerante porque o nome exato do método pode variar no projeto.
    Ajuste aqui se necessário.
    """
    normalized = _normalize_identifier(client_name)
    if normalized in customer_cache:
        return customer_cache[normalized]

    candidates = [
        ("find_customer_id_by_name", (normalized,)),
        ("find_client_id_by_name", (normalized,)),
        ("find_id_by_name", (normalized,)),
        ("find_first_by", ({"cln_name": normalized},)),
        ("find_first_by", ({"company_name": normalized},)),
        ("find_first_by", ({"name": normalized},)),
    ]

    customer_id = None

    for method_name, args in candidates:
        method = getattr(repo_company, method_name, None)
        ...
        try:
            ...
            if isinstance(result, dict):
                customer_id = (
                    _safe_repo_int(result.get("cln_customer_id"))
                    or _safe_repo_int(result.get("customer_id"))
                    or _safe_repo_int(result.get("id"))
                    or _safe_repo_int(result.get("cli_id"))
                )
            else:
                customer_id = _safe_repo_int(result)
    ...
    customer_cache[normalized] = customer_id
    return customer_id
...
def _resolve_product_id(license_name: Any, product_cache: Dict[str, Optional[int]]) -> Optional[int]:
    ...
    candidates = [
        ("find_product_id_by_name", (normalized,)),
        ("find_id_by_name", (normalized,)),
        ("find_ids_by", ({"prd_description": normalized},)),
        ("find_first_by", ({"prd_description": normalized},)),
        ("find_first_by", ({"prd_name": normalized},)),
        ("find_first_by", ({"name": normalized},)),
    ]
    ...
    for method_name, args in candidates:
        method = getattr(repo_product, method_name, None)
        if not callable(method):
            continue
        ...
        try:
            ...
            if isinstance(result, list) and result:
                product_id = _safe_repo_int(result[0])
            elif isinstance(result, dict):
                product_id = (
                    _safe_repo_int(result.get("prd_id"))
                    or _safe_repo_int(result.get("product_id"))
                    or _safe_repo_int(result.get("id"))
                )
            else:
                product_id = _safe_repo_int(result)
...
def _process_file(source_file: Path) -> None:
    ...
    try:
        ...
        summary = (
            ...
            f"sucesso={len(result.success_row_numbers)} | "
            f"falhas={len(result.failed_rows)} | "
            f"remanescente={len(remaining_rows)}"
        )

        _safe_log(log_path, result, summary)
        _register_import_log(source_file, "success", summary)
...
def run_import(file_name: Optional[str] = None) -> None:
    """
