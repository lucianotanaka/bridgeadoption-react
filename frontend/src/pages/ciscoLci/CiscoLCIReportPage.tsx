...
function StageTable({
  ...
  page: number;
  pageSize: number;
  onRowClick?: (row: LCIStageRow) => void;
  canClick?: boolean;
}) {
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-xs">
        ...
        <tbody>
          {pageRows.map((r, i) => (
            <tr
              key={`${r.lci_stage_id}-${i}`}
              onClick={() => canClick && onRowClick?.(r)}
              className={`border-b border-gray-100 dark:border-gray-800 transition-colors ${canClick ? "cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20" : "hover:bg-gray-50 dark:hover:bg-gray-800"}`}
            >
...
type StageFilter = "all" | "approved" | "awaiting" | "ongoing" | "lost";
...
const EXPORT_TABS: { key: Exclude<StageFilter, "all">; sheetName: string }[] = [
...
export default function CiscoLCIReportPage({ fy: selectedFY }: { fy: number }) {
  ...
  // ─── Cascading filter options (Client, Solution, Use Case, Task WS) ────────
  ...
  }, [stages, filterSolution, filterUseCase, filterTaskWs]);
  ...
  }, [stages, filterClient, filterUseCase, filterTaskWs]);
  ...
  }, [stages, filterClient, filterSolution, filterTaskWs]);
  ...
  const taskWsOptions = useMemo(() => {
    ...
    return [...new Set(base.map((r) => r.lci_ws ?? ""))].filter(Boolean).sort();
  ...
  const filteredStages = useMemo(
    ...
    [stages, filterClient, filterSolution, filterUseCase, filterTaskWs]
  );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex gap-2 border-b border-gray-200 dark:border-gray-700 pb-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 text-xs font-medium rounded-t-lg transition-colors ${overviewTab === "financial" ? "bg-blue-600 text-white shadow-sm" : "bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
            >
              <DollarSign size={13} /> {t("adoption.ciscoLci.financialOverview")} — FY {selectedFY}
            </button>
            <button
              onClick={() => setOverviewTab("operational")}
              className={`flex items-center gap-1.5 px-4 py-2 text-xs font-medium rounded-t-lg transition-colors ${overviewTab === "operational" ? "bg-blue-600 text-white shadow-sm" : "bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
            >
          ...
          {overviewTab === "financial" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                <KPICard label={t("adoption.ciscoLci.totalEligibles")} value={fmtUSD(totalEligibles)} accent="blue" tooltipLeft tooltip={t("adoption.ciscoLci.tooltipTotalEligibles")} />
                <KPICard label={t("adoption.ciscoLci.potential")} value={fmtUSD(totalPotentialNew)} accent="blue" tooltipLeft tooltip={t("adoption.ciscoLci.tooltipTotalPotential")} />
                <KPICard label={t("adoption.ciscoLci.totalOptIn")} value={fmtUSD(reportData?.total_eligibles?.total_opt_in ?? s.fin_potential)} accent="blue" tooltip={t("adoption.ciscoLci.tooltipTotalOptIn")} />
                <KPICard label={t("adoption.ciscoLci.captured")} value={fmtUSD(s.fin_approved)} accent="green" tooltip={t("adoption.ciscoLci.tooltipTotalCaptured")} />
                <KPICard label={t("adoption.ciscoLci.lost")} value={fmtUSD(s.fin_lost)} accent="red" tooltip={t("adoption.ciscoLci.tooltipLost")} />
                <KPICard label={t("adoption.ciscoLci.conversionRate")} value={`${(s.fin_conversion_rate * 100).toFixed(1)}%`} accent={s.fin_conversion_rate >= 0.7 ? "green" : "yellow"} tooltip={t("adoption.ciscoLci.tooltipConversionRate")} />
              </div>
                ...
                return (
                  <>
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                      ...
                      <Plot
                        data={[
                          { type: "bar" as const, name: t("adoption.ciscoLci.optIn"), orientation: "h" as const, y: tracks, x: sortedSolutions.map((r) => r.opt_in_value), marker: { color: "#a78bfa" }, text: sortedSolutions.map((r) => { const pct = bd.total_eligibles > 0 ? ((r.opt_in_value / bd.total_eligibles) * 100) : 0; return `${fmtUSD(r.opt_in_value)} (${pct < 0.1 ? "<0.1" : pct.toFixed(1)}%)`; }), textposition: "outside" as const },
                          { type: "bar" as const, name: "Potential", orientation: "h" as const, y: tracks, x: sortedSolutions.map((r) => r.potential_value), marker: { color: "#34d399" }, text: sortedSolutions.map((r) => { const pct = bd.total_eligibles > 0 ? ((r.potential_value / bd.total_eligibles) * 100) : 0; return `${fmtUSD(r.potential_value)} (${pct < 0.1 ? "<0.1" : pct.toFixed(1)}%)`; }), textposition: "outside" as const },
                          { type: "bar" as const, name: t("adoption.ciscoLci.eligible"), orientation: "h" as const, y: tracks, x: sortedSolutions.map((r) => r.eligible_value), marker: { color: "#60a5fa" }, text: sortedSolutions.map((r) => { const pct = bd.total_eligibles > 0 ? ((r.eligible_value / bd.total_eligibles) * 100) : 0; return `${fmtUSD(r.eligible_value)} (${pct < 0.1 ? "<0.1" : pct.toFixed(1)}%)`; }), textposition: "outside" as const },
                        ]}
              ...
              {forecastClientQuery.data?.top5_achieved && forecastClientQuery.data.top5_achieved.length > 0 && (
                <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                  ...
                  <Plot
                    data={[{ type: "bar" as const, orientation: "h" as const, y: forecastClientQuery.data.top5_achieved.map((d) => d.client), x: forecastClientQuery.data.top5_achieved.map((d) => d.value), marker: { color: "#005B96" }, text: forecastClientQuery.data.top5_achieved.map((d) => d.value_fmt), textposition: "outside" as const }]}
              ...
              {lostJust.length > 0 && (
                <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                  ...
                  <Plot
                    data={[{ type: "bar" as const, orientation: "h" as const, y: lostJust.map((d) => d.justification || "Not Specified"), x: lostJust.map((d) => d.count), marker: { color: "#F04438" }, text: (() => { const totalCount = lostJust.reduce((s, d) => s + d.count, 0); return lostJust.map((d) => { const pct = totalCount > 0 ? ((d.count / totalCount) * 100).toFixed(0) : "0"; return `${d.count} tasks — ${fmtUSD(d.value)} — ${pct}%`; }); })(), textposition: "outside" as const }]}
                    layout={{ paper_bgcolor: isDark ? "#111827" : "white", plot_bgcolor: isDark ? "#111827" : "white", font: { color: isDark ? "#d1d5db" : "#374151", size: 11 }, height: Math.max(320, lostJust.length * 40 + 100), xaxis: { title: "# Tasks", automargin: true, range: [0, Math.max(...lostJust.map((d) => d.count)) * 2.2] }, yaxis: { autorange: "reversed" as const, automargin: true }, showlegend: false, margin: { t: 20, b: 60, l: 240, r: 20 } }}
                    useResizeHandler style={{ width: "100%" }} config={{ displayModeBar: false }}
                  />
                </div>
              )}
      ...
      {/* Filters — cascade: Client, Solution, Use Case, Task WS */}
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <MultiSelectDropdown label="Task WS" options={taskWsOptions} selected={filterTaskWs} onChange={(v) => { setFilterTaskWs(v); setTablePage(1); }} placeholder="All Task WS" />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between gap-2 mb-4 border-b border-gray-200 dark:border-gray-700 pb-3">
          <div className="flex items-center gap-2 flex-wrap">
            {tabs.map((tab) => (
              <button key={tab.key} onClick={() => { setActiveTab(tab.key); setTablePage(1); setFilterClient([]); setFilterSolution([]); setFilterUseCase([]); setFilterTaskWs([]); }}
        ...
        {stagesQuery.isLoading ? (
          ...
          <>
            {hasTaskPermission && (
              <p className="flex items-center gap-1 text-[10px] text-blue-500 dark:text-blue-400 mb-1">
                <Info size={11} />
                {t("adoption.ciscoLci.clickRowHint")}
              </p>
            )}
            <StageTable
              rows={filteredStages}
